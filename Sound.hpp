...
public:
    static void Sound();
    static int Sound2();
    static void MySQL_Sound(string filename, string float_string, string current_time, string transcription, string sound_recognition, string sound_hash, string analysis);
    //static void SpeechCommand(vector<string> words);
    ...
    static string CombineWAV(vector<string> wav_files, string sound_directory);
    ...
    static std::vector<string> LoadClassLabels(const std::string& filePath);
    static void get_audio(int seconds);

    vector<float> pcmf32_cur;
    int numSamples;
};
